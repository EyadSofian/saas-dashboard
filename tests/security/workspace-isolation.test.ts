...
beforeAll(async () => {
  ...
  for (const id of [
    ...
    await runMigration(adminPool, id);
  }
...
describe("canonical layer is workspace-scoped too", () => {
  it("does not leak fact rows across workspaces", async () => {
    // The canonical tables are where customer business data actually lands, so
    // they need the same proof as the control-plane tables.
    const generation = await adminPool.query<{ id: string }>(
      "INSERT INTO data_generations (workspace_id, status) VALUES ($1,'published') RETURNING id",
      [ALPHA_WS],
    );
    const betaGeneration = await adminPool.query<{ id: string }>(
      "INSERT INTO data_generations (workspace_id, status) VALUES ($1,'published') RETURNING id",
      [BETA_WS],
    );
    await adminPool.query(
      `INSERT INTO fact_invoice (workspace_id, generation_id, source_id, amount_total)
       VALUES ($1,$2,1,1000), ($3,$4,1,9999)`,
      [ALPHA_WS, generation.rows[0].id, BETA_WS, betaGeneration.rows[0].id],
    );

    const amounts = await asWorkspace(ALPHA_WS, async (client) => {
      const r = await client.query("SELECT amount_total FROM fact_invoice");
      return r.rows.map((x) => Number(x.amount_total));
    });
    // Same source_id in both workspaces: only alpha's row is visible.
    expect(amounts).toEqual([1000]);
  });

  it("does not leak semantic mappings across workspaces", async () => {
    const rows = await asWorkspace(ALPHA_WS, async (client) => {
      const r = await client.query("SELECT * FROM semantic_field_mappings");
      return r.rows;
    });
    expect(rows.every((r) => r.workspace_id === ALPHA_WS)).toBe(true);
  });
