...
export const Route = createFileRoute("/api/v1/discovery")({
  server: {
    handlers: {
      // Current discovery state: the latest ready snapshot (which survives a
      // failed run) plus the most recent run's outcome, reported separately.
      GET: async ({ request }) => {
      ...
      // Starts a discovery run. Returns immediately: a full scan targets under
      // 10 minutes, far too long to hold a request open.
      POST: async ({ request }) => {
        ...
        try {
          ...
          // Failures are recorded by the job itself; this prevents an unhandled
          // rejection from taking down the process.
