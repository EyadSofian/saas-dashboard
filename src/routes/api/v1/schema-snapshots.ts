...
export const Route = createFileRoute("/api/v1/schema-snapshots")({
  server: {
    handlers: {
      ...
      //   ?model=<name>  returns that model's fields
      //   otherwise      returns the snapshot header and its model list
      //
      // Field labels and help text are customer-controlled strings. They are
      // returned as data for display and must never be treated as instructions
      // by anything downstream (THREAT_MODEL T4).
      GET: async ({ request }) => {
