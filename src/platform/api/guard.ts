...
//   1. feature flag  — a disabled feature is indistinguishable from absent (404)
//   2. session       — not signed in is 401
//   3. membership    — signed in but not a member is 403
//   4. permission    — a member without the role is 403
//
// The workspace id arrives from the client, so step 3 is what actually decides:
// membership is the authority, the header is only a request (INV-5).
...
export async function requireWorkspace(request: Request): Promise<GuardSuccess | GuardFailure> {
  if (!FLAGS.workspaces()) {
    // 404, not 403: a disabled feature should not advertise its existence.
    return { ok: false, response: errorResponse("Not found.", 404) };
  }
